...
describe('multi-part().buffer()', function () {
  let server;
  this.timeout(10000);
  before((done) => {
  ...
  describe('append vinyl', () => {
    it('should be ok', async () => {
      ...
      const body = await form.buffer();
      return got.post('http://127.0.0.1:4000', { headers: form.getHeaders(), body }).then((res) => {
        should(JSON.parse(res.body)).be.eql({ filename: 'anon.jpg', mime: 'application/octet-stream', fields: {} });
      });
    });
    ...
    it('should be ok', (done) => {
      ...
      const req = http.request({
        ...
        res.on('end', () => {
          should(JSON.parse(Buffer.concat(chunks).toString('utf8'))).be.eql({ filename: 'anon.jpg', mime: 'application/octet-stream', fields: {} });
          done();
        });
      });
      form.buffer().then((body) => {
  ...
  describe('append array', () => {
    it('should be ok', async () => {
      ...
      const body = await form.buffer();
      return got.post('http://127.0.0.1:4000', { headers: form.getHeaders(), body }).then((res) => {
        should(JSON.parse(res.body)).be.eql({ filename: 'anon.jpg', mime: 'application/octet-stream', fields: { array: ['arr', 'arr1', 'arr2', 'arr3', '0', ''] } });
      });
    });
    ...
    it('should be ok', (done) => {
      ...
      const req = http.request({
        ...
        res.on('end', () => {
          should(JSON.parse(Buffer.concat(chunks).toString('utf8'))).be.eql({ filename: 'anon.jpg', mime: 'application/octet-stream', fields: { array: ['arr', 'arr1', 'arr2', 'arr3', '0', ''] } });
          done();
        });
      });
      form.buffer().then((body) => {
  ...
  describe('append stream', () => {
    describe('chunked', () => {
      it('should be ok', async () => {
        ...
        const body = await form.buffer();
        return got.post('http://127.0.0.1:4000', { headers: form.getHeaders(), body }).then((res) => {
          should(JSON.parse(res.body)).be.eql({ filename: 'fixture.jpg', mime: 'application/octet-stream', fields: { field: ['12345', '0'] } });
        });
      });
      ...
      it('should be ok', async () => {
        ...
        const body = await form.buffer();
        return got.post('http://127.0.0.1:4000', { headers: form.getHeaders(), body }).then((res) => {
          should(res.body).be.eql('{"filename":"a.jpg","mime":"application/octet-stream","fields":{"field":["12345","0"]}}');
        });
      });
  ...
  describe('append buffer', () => {
    ...
    describe('chunked', () => {
      ...
      it('should be ok', async () => {
        ...
        const body = await form.buffer();
        return got.post('http://127.0.0.1:4000', { headers: form.getHeaders(), body }).then((res) => {
          should(res.body).be.eql('{"filename":"file.bin","mime":"image/jpeg","fields":{"field":["12345","0"]}}');
        });
      });
      ...
      it('should be ok', (done) => {
        ...
        const req = http.request({
          ...
          res.on('end', () => {
            should(Buffer.concat(chunks).toString('utf8')).be.eql('{"filename":"file.bin","mime":"application/octet-stream","fields":{"field":["12345","0"]}}');
            done();
          });
        });
        form.buffer().then((body) => {
